...
object Day06Part1:
    ...
    def findBeginningMessagePositionWithMarkerOfSize(input: String, markerSize: Int) =
        ...
        else findBeginningMessagePosition(Queue[Char](input.substring(0, markerSize).toList :_*), markerSize, input.substring(markerSize).toList)

    @tailrec
    private def findBeginningMessagePosition(current: Queue[Char], lastMarkerCharPosition: Int, remaining: List[Char]): Int =
